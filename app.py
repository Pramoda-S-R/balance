...
from models import db, User, Transaction, MonthlyBudget
...
def get_current_cycle_start(today=None):
    if today is None:
        today = datetime.date.today()
    
    if today.day >= 8:
        return datetime.date(today.year, today.month, 8)
    else:
        # Go back to previous month's 8th
        ...
        return datetime.date(last_of_prev_month.year, last_of_prev_month.month, 8)

@app.route('/')
@login_required
def index():
    cycle_start = get_current_cycle_start()
    budget = MonthlyBudget.query.filter_by(user_id=current_user.id, month_start_date=cycle_start).first()
...
def register():
    if request.method == 'POST':
        ...
        # Set initial budget
        cycle_start = get_current_cycle_start()
        budget = MonthlyBudget(user_id=user.id, month_start_date=cycle_start, salary_credited=float(salary))
...
def set_budget():
    ...
    if request.method == 'POST':
        salary = request.form.get('salary')
        budget = MonthlyBudget(user_id=current_user.id, month_start_date=cycle_start, salary_credited=float(salary))
        db.session.add(budget)
        db.session.commit()
        return redirect(url_for('index'))
...
def download_statement():
    ...
    pdf.cell(0, 20, "HAKOGANE - FINANCIAL STATEMENT", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    
    pdf.set_font("Helvetica", 'I', 10)
    pdf.set_text_color(138, 141, 164)
    pdf.cell(0, 10, f"Generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.ln(10)
    
    # Summary Info
    pdf.set_text_color(240, 240, 245)
    pdf.set_font("Helvetica", 'B', 14)
    pdf.cell(0, 10, f"Operator: {current_user.username}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.cell(0, 10, f"Cycle Start: {cycle_start.strftime('%d %b %Y')}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(5)
    ...
    pdf.cell(90, 10, f"Rs. {budget.salary_credited:.2f}", border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    
    pdf.set_text_color(255, 85, 85)
    pdf.cell(90, 10, "Total Consumption")
    pdf.cell(90, 10, f"Rs. {total_spent:.2f}", border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    
    pdf.set_text_color(0, 255, 170)
    pdf.cell(90, 10, "Available Reserve")
    pdf.cell(90, 10, f"Rs. {remaining_balance:.2f}", border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.cell(40, 10, "Amount (INR)", border=1, fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    for t in transactions:
        ...
        pdf.cell(40, 10, f"{t.amount:.2f}", border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    
    pdf.ln(20)
    pdf.set_font("Helvetica", 'I', 8)
    pdf.set_text_color(138, 141, 164)
    pdf.cell(0, 10, "End of Hakogane Protocol Statement.", align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
